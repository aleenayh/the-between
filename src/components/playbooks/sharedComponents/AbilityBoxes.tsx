    ...
    <div className={`flex justify-center gap-1 mx-0 md:mx-auto`}>
      {orderAbilities(stats).map(({ ability, value }) =>
        abbreviate ? (
          ...
          <AbilityBox ability={ability} value={value} key={ability} />
        ),
      )}
    </div>
  )
...
function StaticAbilityBox({ ability, value }: { ability: keyof AbilityType; value: number }) {
  return (
    <div className="flex flex-col gap-1 rounded-lg border border-theme-border-accent p-1 bg-theme-bg-secondary justify-center items-center min-w-[10%]">
      <h4 className="text-theme-text-muted text-[0.5rem] truncate max-w-full whitespace-nowrap overflow-hidden text-ellipsis">
        {ability.slice(0, 4)}
      </h4>
...
const order = ["vitality", "composure", "reason", "presence", "sensitivity"]
...
function DieComponent({ dice, id, index }: { dice: Die[]; id: string; index: number }) {
  const die = dice.find((die) => die.id === `${id}-${index}`)
  if (!die) {
    return null
  }
  return (
    <div key={die.id} className={`diceBase ${die.exclude ? "diceExcluded" : ""} ${die.isRolling ? "diceRolling" : ""}`}>
      <AnimatePresence>
        {die.isRolling ? (
