...
export function CharacterOverview() {
  ...
  const myCharacter = players.find((player) => player.id === user.id)?.character
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden gap-3">
      ...
      <div className="flex flex-1 min-h-0 mb-6 md:mb-0 overflow-hidden gap-1 max-w-[1400px] mx-auto">
...
function CharacterCreationStarter({ onCollapse }: { onCollapse: () => void }) {
  ...
  const activeHunters = gameState.players
        ...
        <div className="flex flex-col gap-4 h-full">
          ...
          <div className="flex flex-col md:grid md:grid-cols-2 gap-2 overflow-y-auto">
            {Object.values(playbookKeys).map((playbookKey) => {
              ...
              const activeHunter = activeHunters.find((Hunter) => Hunter.playbook === playbookKey)
              return (
                <GlassyButton
                  key={playbookKey}
                ...
                >
                  ...
                  <span className="text-sm text-theme-text-muted italic hover:text-theme-text-accent">
                    {activeHunter ? `(Played by ${activeHunter.name})` : ""}
                  </span>
                </GlassyButton>
              )
            })}
